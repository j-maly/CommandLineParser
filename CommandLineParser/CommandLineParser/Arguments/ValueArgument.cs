...
namespace CommandLineParser.Arguments
{
    ...
    public class ValueArgument<TValue>: Argument, IValueArgument, IArgumentWithDefaultValue
    {
        ...
        private ConvertValueDelegate<TValue> convertValueHandler;
        ...
        {
            get { return convertValueHandler; }
            set { convertValueHandler = value; }
        }
        ...
        internal override void Parse(IList<string> args, ref int i)
        {
            ...
            if (args.Count - 1 < i)
            {
                throw new CommandLineArgumentException(
                   String.Format(Messages.EXC_ARG_VALUE_MISSING2, Name), Name
                   );
            }
            if (args[i].StartsWith("-"))
            {
                throw new CommandLineArgumentException(
                    String.Format(Messages.EXC_ARG_VALUE_MISSING, Name, args[i]), Name
                    );
            }
            stringValue = args[i];
            ...
            if (!AllowMultiple)
                ...
                Values.Add(Convert(stringValue));
        ...
        protected virtual TValue DefaultConvert(string stringValue)   
        {
            ...
            try
            {
                ...
                if (valueType == typeof(byte)) return (TValue)(object)byte.Parse(stringValue, cultureInfo);
                if (valueType == typeof(sbyte)) return (TValue)(object)sbyte.Parse(stringValue, cultureInfo);
                if (valueType == typeof(double)) return (TValue)(object)double.Parse(stringValue, cultureInfo);
                if (valueType == typeof(float)) return (TValue)(object)float.Parse(stringValue, cultureInfo);
                if (valueType == typeof(uint)) return (TValue)(object)uint.Parse(stringValue, cultureInfo);
                if (valueType == typeof(ulong)) return (TValue)(object)ulong.Parse(stringValue, cultureInfo);
                if (valueType == typeof(ushort)) return (TValue)(object)ushort.Parse(stringValue, cultureInfo);
                ...
                if (mi != null)
                {
                    ...
                    if (mi.IsStatic
                        ...
                        return (TValue) mi.Invoke(null, new object[] {stringValue, cultureInfo});
                }
            }
        ...
        {
            if (!AllowMultiple)
                Console.WriteLine(Messages.EXC_ARG_VALUE_PRINT, Name, stringValue, value, typeof(TValue).Name);
            else
            {
